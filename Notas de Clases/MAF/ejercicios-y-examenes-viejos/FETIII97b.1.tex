...
\centerline{\sl Examen 1b}\Par

%%Begin InstantTeX Picture
\let\picnaturalsize=N
\def\picsize{1.0in}
\def\picfilename{uam.eps}
%If you do not have the picture file add:
%\let\nopictures=Y
%to the beginning of the file.
\ifx\nopictures Y\else{\ifx\epsfloaded Y\else\input epsf \fi
...
\ni 1) Calcular la siguiente integral

 $$  \int_0^\infty {x^s\over(e^x+1)}dx = s!\,(1-2^{-s})\zeta(s + 1)$$

\ni 2) Demostrar la relaci\'on siguiente 

$$ \int_{-1}^1 (1-x^2)^{-1/2}x^{2n}dx = \cases{{\pi}& Si $n=0$\cr
\pi {(2n-1)!!\over (2n)!!}  & Si $n= 1,2,3...$}$$
